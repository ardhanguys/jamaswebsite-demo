...
const Header = () => {
  ...
  return (
    <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300", isScrolled ? "bg-background/95 backdrop-blur-md shadow-elegant" : "bg-transparent")}>
        ...
        <div className={cn("md:hidden overflow-hidden transition-all duration-300", isMobileMenuOpen ? "max-h-96 pb-6" : "max-h-0")}>
          <ul className="flex flex-col gap-4 bg-background rounded-lg p-4 shadow-elegant">
            {navLinks.map(link => (
              <li key={link.path}>
                {link.path.startsWith("/#") ? (
                  <Link to={link.path} onClick={() => handleNavClick(link.path)} className="block font-medium text-foreground hover:text-accent transition-colors py-2">
                    {link.name}
                  </Link>
                ) : (
                  <Link to={link.path} className={cn("block font-medium text-foreground hover:text-accent transition-colors py-2", location.pathname === link.path && "text-accent")}>
                    {link.name}
                  </Link>
                )}
              </li>
